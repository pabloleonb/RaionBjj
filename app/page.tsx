...
/**
 * ===================================================================================
 * RAION BJJ - OFFICIAL WEBSITE (ELITE EDITION)
 * ===================================================================================
 * Este es el componente principal que renderiza toda la Single Page Application.
 * Utiliza Next.js, TailwindCSS para estilos, y Framer Motion para animaciones.
 */
...
import { 
  ...
  Shield, Target, Activity, ChevronRight, ChevronLeft,
  Plus, Minus, Quote, HelpCircle, MessageCircle, Menu, X, Camera
} from 'lucide-react';

// ==========================================
// 1. CONFIGURACIÓN DE DATOS (DATA CONFIG)
// ==========================================
// Aquí puedes cambiar los textos, horarios y fotos sin tocar el diseño.

// Horarios de clases
const SCHEDULE = [
  { time: "07:30 - 08:30", days: ["BJJ (Fundamentos)", "—", "BJJ (Fundamentos)", "—", "BJJ (Fundamentos)", "—"] },
...
// Datos de Instructores (Elite Team)
...
// Fotos del carrusel de Instagram
const IG_PHOTOS = [
  "/img/Adultos2.png",
  "/img/NiñosGrupal.png",
  "/img/juvenil2.png",
...
// ==========================================
// 2. COMPONENTES INTERNOS (SUB-COMPONENTS)
// ==========================================

/**
 * FAQItem: Maneja el estado de apertura/cierre de cada pregunta frecuente.
 */
...
// ==========================================
// 3. COMPONENTE PRINCIPAL (MAIN COMPONENT)
// ==========================================
export default function RaionFinalEdition() {
  // Estados Globales (Loading, Menú, Day Picker, Modal de Instructores)
  ...
  // Lógica de Scroll Progress (Línea roja arriba)
  ...
  // Ciclo de Vida: Preloader y Carrusel IG
  useEffect(() => {
    setMounted(true);
  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen selection:bg-[#E50000] selection:text-white font-sans overflow-x-hidden">
      ...
      {/* --- PRELOADER: El video de carga inicial --- */}
      <AnimatePresence>
        {loading && (
          <motion.div exit={{ y: "-100%", transition: { duration: 1, ease: [0.76, 0, 0.24, 1] } }} className="fixed inset-0 z-[100] bg-black flex items-center justify-center overflow-hidden">
      ...
      <div className={loading ? "h-screen overflow-hidden" : "relative"}>
        ...
        <header className="fixed top-0 w-full z-50 bg-[#050505] border-b border-white/5 shadow-2xl transition-all duration-300">
          <div className="max-w-[1600px] mx-auto py-6 px-6 md:px-12 flex justify-between items-center">
            {/* Logo Raion */}
            <div className="flex items-center gap-4 md:gap-6 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
               ...
               <div className="flex flex-col">
                  <span className="font-display text-2xl md:text-3xl font-black tracking-tighter leading-none italic uppercase">RAION BJJ</span>
            ...
            <div className="hidden lg:flex items-center gap-16">
               <nav className="flex items-center gap-12 text-[10px] font-black uppercase tracking-[0.4em] text-white/50">
                  {[
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <motion.div 
                ...
                animate={{ x: 0 }}
                exit={{ x: "100%" }}
                className="fixed inset-0 bg-black z-[100] lg:hidden flex flex-col p-8 gap-6 text-white pt-24"
              >
                <div className="flex items-center gap-4 mb-8">
                   <img src="/img/Logo_Raion.png" className="h-10 w-auto" alt="Raion" />
                   <span className="font-display text-xl font-black italic uppercase">RAION BJJ</span>
                </div>
                ...
                {[
                  ...
                  <a key={item.label} href={item.href} onClick={() => setIsMenuOpen(false)} className="text-base font-display font-black uppercase italic tracking-widest hover:text-[#E50000] transition-colors border-b border-white/10 pb-2">
                    {item.label}
                  </a>
                ))}
                <a href="https://instagram.com/raion_bjj" target="_blank" onClick={() => setIsMenuOpen(false)} className="text-base font-display font-black uppercase italic tracking-widest hover:text-[#E50000] transition-colors border-b border-white/10 pb-2 flex items-center gap-3">
                   INSTAGRAM
                </a>
        ...
        {/* Barra de progreso de lectura (roja) */}
        <motion.div className="fixed top-0 left-0 right-0 h-[2px] bg-[#E50000] z-[60] origin-left shadow-[0_0_10px_#FF0000]" style={{ scaleX }} />

        {/* --- HERO SECTION: Portada con video y título monumental --- */}
        <section className="relative min-h-screen flex flex-col justify-center px-6 md:px-24 pt-32 pb-20">
          ...
          <div className="relative z-10 max-w-7xl">
             <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
                ...
                <h1 className="font-display text-5xl md:text-8xl lg:text-[10vw] uppercase italic font-black leading-[0.8] mb-12 tracking-tighter leading-none">
        ...
        {/* --- BRAND VISUAL (MOBILE ONLY): Sello Raion blanco --- */}
        ...
        {/* --- PHILOSOPHY: Manifiesto y Boxes de valores --- */}
        <section id="about" className="pt-4 md:pt-40 pb-24 md:pb-40 px-6 md:px-24 bg-white text-black">
           <div className="max-w-[1600px] mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                 {/* Box principal: El Manifiesto */}
                 <div className="md:col-span-8 bg-[#F5F5F5] border-0 md:border-2 border-[#E50000] rounded-[2rem] md:rounded-[3rem] p-8 md:p-20 flex flex-col justify-between group overflow-hidden relative shadow-xl">
                    <div className="absolute top-0 right-0 p-20 opacity-5 group-hover:opacity-10 transition-opacity"><img src="/img/icono_leon_sabio.png" className="w-80" alt="Icon" /></div>
                    
                    <div className="flex justify-between items-start mb-12">
                       <span className="text-[#E50000] font-black text-[10px] tracking-[0.5em] block uppercase">01 / MANIFIESTO</span>
                    </div>
                    
                    <div className="flex flex-col items-start gap-6 mb-12">
                       <h2 className="text-4xl md:text-7xl lg:text-8xl font-display font-black uppercase italic leading-[0.9] tracking-tighter">
                 ...
                 {/* Boxes laterales de valores */}
                 <div className="md:col-span-4 flex flex-col gap-6">
                    <div className="flex-1 bg-white border-2 border-black/5 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 flex flex-col justify-between hover:border-[#E50000] hover:text-[#E50000] transition-all duration-700 shadow-lg group active:border-[#E50000]">
        ...
        {/* --- PROGRAMAS: Listado de categorías (Cubs, Cadetes, Adultos) --- */}
        <section id="programs" className="bg-black text-white py-24 md:py-40 border-b border-white/5">
           <div className="max-w-[1600px] mx-auto px-6 md:px-24 mb-16 md:mb-24 text-center md:text-left">
              ...
              <h2 className="font-display text-4xl md:text-8xl lg:text-[10rem] uppercase italic font-black leading-none tracking-tighter">PROGRAMAS</h2>
           </div>
           <div className="space-y-8 px-4 md:px-8 max-w-[1600px] mx-auto">
              {PROGRAMS.map((prog, i) => (
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} key={i} className="group relative overflow-hidden bg-[#0A0A0A] rounded-[2rem] md:rounded-[3rem] border border-white/5 shadow-2xl">
                   <div className="relative aspect-[4/5] md:aspect-[21/9] w-full">
                      ...
                      <div className="absolute bottom-0 left-0 w-full p-6 md:p-24">
                         <h4 className="font-display text-3xl md:text-8xl uppercase italic mb-2 md:mb-6 group-hover:text-[#E50000] transition-colors duration-500 leading-none">{prog.title}</h4>
        ...
        {/* --- INSTRUCTOR LÍDER: Prof. Pablo León --- */}
        <section id="instructors" className="py-24 md:py-40 bg-black relative overflow-hidden">
           <div className="max-w-[1600px] mx-auto px-6 md:px-24">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                 ...
                 <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative group">
                    ...
                    <img src="/img/Foto Perfil - completa.jpeg" className="relative rounded-[3rem] w-full h-auto grayscale hover:grayscale-0 transition-all duration-1000 shadow-2xl border border-white/5" alt="Prof. Pablo León" />
                    <div className="absolute bottom-6 left-6 right-6 md:right-auto bg-black/60 backdrop-blur-2xl p-6 md:p-10 border border-white/10 rounded-[2.5rem] shadow-2xl group-hover:border-[#E50000]/50 transition-all duration-700">
                       ...
                       <h3 className="text-2xl md:text-4xl font-display font-black italic uppercase tracking-tighter">Prof. Pablo León</h3>
        ...
        {/* --- ELITE TEAM: Grilla de Instructores asistentes --- */}
        <section className="py-24 md:py-40 bg-white text-black overflow-hidden border-y border-black/5">
           <div className="max-w-[1600px] mx-auto px-6 md:px-24 mb-16 md:mb-24 flex flex-col md:flex-row justify-between items-end gap-10">
              <div>
                 <span className="text-[#E50000] font-black text-[10px] tracking-[0.6em] uppercase mb-6 block italic">Squad 01 / Elite Support</span>
                 <h2 className="font-display text-5xl md:text-8xl uppercase italic font-black leading-[0.85] tracking-tighter">ELITE <br /> <span className="text-[#E50000]">TEAM.</span></h2>
              </div>
              <p className="max-w-md text-lg italic font-light text-black/50 leading-relaxed border-l border-[#E50000] pl-8">Nuestro equipo de apoyo garantiza que cada clase mantenga el estándar de excelencia técnica que define a Raion.</p>
           </div>
                 ...
                 {INSTRUCTORS_DATA.map((member, i) => (
                    ...
                    >
                       ...
                       <div className="absolute bottom-0 left-0 w-full p-8 text-white">
                          <span className="text-[#E50000] font-black text-[8px] tracking-[0.3em] uppercase block mb-2 italic">{member.role}</span>
                          <h4 className="text-2xl font-display font-black italic uppercase tracking-tighter">{member.name}</h4>
                       </div>
                    </motion.div>
                 ))}
              </div>
           ...
           {/* --- MODAL DE INSTRUCTOR (DOSSIER): Aparece al hacer clic en un miembro del equipo --- */}
           <AnimatePresence>
             {selectedInstructor && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-12 bg-black/90 backdrop-blur-xl" onClick={() => setSelectedInstructor(null)}>
                 <motion.div initial={{ scale: 0.9, y: 50, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 50, opacity: 0 }} className="bg-[#0A0A0A] w-full max-w-5xl rounded-[3rem] md:rounded-[4rem] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(229,0,0,0.2)] flex flex-col md:flex-row max-h-[90vh] overflow-y-auto no-scrollbar" onClick={(e) => e.stopPropagation()}>
                    {/* Imagen del modal */}
                    <div className="w-full md:w-1/2 h-[300px] md:h-auto relative shrink-0">
                    ...
                    {/* Contenido del modal */}
                    <div className="w-full md:w-1/2 p-10 md:p-20 flex flex-col justify-center relative text-white">
                       <button onClick={() => setSelectedInstructor(null)} className="absolute top-8 right-8 text-white/20 hover:text-white transition-colors"><X size={32} /></button>
                       <div className="px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-white shadow-2xl mb-8 w-fit border border-white/10 italic" style={{ backgroundColor: selectedInstructor.beltColor }}>{selectedInstructor.belt}</div>
                       <h3 className="text-3xl md:text-6xl font-display font-black italic uppercase tracking-tighter mb-4">{selectedInstructor.name}</h3>
                       <p className="text-[#E50000] font-bold text-sm uppercase tracking-widest mb-10 italic">{selectedInstructor.role}</p>
                       <p className="text-white/50 italic font-light leading-relaxed text-base md:text-xl border-l-2 border-[#E50000] pl-8">{selectedInstructor.bio}</p>
                    </div>
                 </motion.div>
               </motion.div>
             )}
        ...
        {/* --- INSTAGRAM VIEWPORT: Carrusel automático High-Tech --- */}
        <section className="py-24 md:py-40 bg-black text-white relative overflow-hidden">
           <div className="max-w-[1600px] mx-auto px-6 md:px-24 mb-20 text-center">
              <div className="flex items-center justify-center gap-4 mb-8">
                 <Camera className="text-[#E50000] animate-pulse" size={40} />
              </div>
              <h2 className="font-display text-4xl md:text-7xl uppercase italic font-black leading-none tracking-tighter">SÍGUENOS EN <span className="text-[#E50000]">VIVO.</span></h2>
              <a href="https://instagram.com/raion_bjj" target="_blank" className="inline-block mt-8 text-white/40 hover:text-[#E50000] transition-colors font-black tracking-[0.4em] text-[10px] uppercase italic">@raion_bjj / Live Technical Feed</a>
           </div>

           <div className="max-w-[800px] mx-auto px-6 relative">
              {/* Marco Tecnológico con bordes rojos */}
              <div className="relative group aspect-[4/5] md:aspect-video rounded-[3rem] overflow-hidden border border-white/10 bg-[#0A0A0A] shadow-[0_0_80px_rgba(229,0,0,0.15)]">
                 ...
                 {/* Imagen animada con AnimatePresence */}
                 <AnimatePresence mode="wait">
                    <motion.img key={activeIgPhoto} initial={{ opacity: 0, scale: 1.1, filter: "grayscale(100%) brightness(50%)" }} animate={{ opacity: 1, scale: 1, filter: "grayscale(0%) brightness(100%)" }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 1.2, ease: [0.76, 0, 0.24, 1] }} src={IG_PHOTOS[activeIgPhoto]} className="absolute inset-0 w-full h-full object-cover" alt="Raion Live" />
                 </AnimatePresence>

                 {/* Capas superiores: REC overlay y Dots */}
                 ...
                 <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-3 z-20">
                    {IG_PHOTOS.map((_, i) => (
                       <button key={i} onClick={() => setActiveIgPhoto(i)} className={`h-1 transition-all duration-500 rounded-full ${activeIgPhoto === i ? 'w-10 bg-[#E50000]' : 'w-4 bg-white/20'}`} />
                    ))}
                 </div>
                 {/* Acción al pasar el mouse */}
                 <a href="https://instagram.com/raion_bjj" target="_blank" className="absolute inset-0 z-30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 bg-black/40 backdrop-blur-sm">
        ...
        {/* --- HORARIOS (WAR ROOM): Desktop (Tabla) y Mobile (Selector de día) --- */}
        <section id="schedule" className="py-24 md:py-40 bg-white text-black relative">
           <div className="max-w-7xl mx-auto px-6 md:px-24 text-center md:text-left mb-16 md:mb-24">
              <h2 className="font-display text-5xl md:text-8xl uppercase italic font-black tracking-tighter leading-none">WAR <br/> <span className="text-[#E50000]">ROOM</span></h2>
           </div>

           {/* Versión Desktop: Tabla completa */}
           <div className="max-w-[1600px] mx-auto px-4 md:px-24 hidden lg:block overflow-x-auto no-scrollbar">
              <div className="min-w-[1000px] border-[2px] border-black rounded-[3rem] overflow-hidden shadow-2xl">
           ...
           {/* Versión Mobile: Selector de un día a la vez */}
           <div className="lg:hidden px-6">
              <div className="flex justify-between items-center mb-10 bg-black text-white p-6 rounded-[2rem] shadow-xl">
                 ...
                 <div className="text-center">
                    <span className="text-[10px] font-black tracking-[0.4em] opacity-50 uppercase mb-2 block italic">Viendo día</span>
                    ...
                    return row.days[activeDay] !== '—' && (
                       <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center justify-between p-8 bg-[#F5F5F5] border border-black/5 rounded-[2.5rem] shadow-sm">
                          <div className="flex flex-col gap-1">
                             <span className="text-[10px] font-black uppercase tracking-widest opacity-40 italic">Hora</span>
                             <span className="text-xl font-black italic tracking-tighter">{displayTime}</span>
                          </div>
                          <div className="text-right">
                             <span className="text-[10px] font-black uppercase tracking-widest text-[#E50000] mb-1 block italic">Disciplina</span>
                             <span className="text-xl font-black italic tracking-tighter uppercase">{displayDiscipline}</span>
                          </div>
                       </motion.div>
                    );
                 })}
        ...
        <section id="faqs" className="py-24 md:py-40 px-6 md:px-24 bg-white text-black border-t border-black/5">
           <div className="max-w-4xl mx-auto">
              <div className="flex items-center gap-4 mb-12">
                 ...
                 <h2 className="text-4xl md:text-6xl font-display font-black uppercase italic tracking-tighter leading-none">FAQS</h2>
              </div>
              <div className="space-y-4">
        ...
        {/* --- TESTIMONIALS (LA MANADA): Carrusel infinito (Marquee) --- */}
        <section className="py-24 md:py-40 bg-[#050505] overflow-hidden border-y border-white/5">
           <div className="max-w-[1600px] mx-auto px-6 md:px-24 mb-16 md:mb-24 text-center md:text-left">
              <span className="text-[#E50000] font-bold uppercase tracking-[0.5em] text-[10px] mb-4 block italic">Comunidad & Resultados</span>
              <h2 className="font-display text-5xl md:text-8xl lg:text-[10rem] uppercase italic font-black leading-none tracking-tighter">LA MANADA</h2>
           </div>
           {/* El marquee usa una animación CSS definida al final del archivo */}
           <div className="flex gap-12 animate-marquee whitespace-nowrap">
              {[...REVIEWS, ...REVIEWS].map((rev, i) => (
                <div key={i} className="inline-block bg-[#0A0A0A] p-12 border border-white/5 w-[320px] md:w-[450px] whitespace-normal italic group hover:border-[#E50000] transition-colors duration-500 rounded-[2rem] shadow-2xl">
                   <Quote className="text-[#E50000]/20 mb-8 group-hover:text-[#E50000]/40 transition-colors" size={48} />
                   <p className="text-white/60 text-xl leading-relaxed mb-10 font-light">"{rev.q}"</p>
        ...
        {/* --- FOOTER: Contacto, Redes y Mapa Interactivo --- */}
        <footer id="contact" className="py-24 md:py-32 px-4 md:px-16 bg-black border-t border-white/5">
           <div className="max-w-[1400px] mx-auto bg-[#0A0A0A] rounded-[4rem] p-8 md:p-16 border border-white/5 overflow-hidden shadow-2xl">
              <div className="grid lg:grid-cols-2 gap-20 relative z-10">
                 {/* Columna Izquierda: Links de Contacto y Formulario */}
                 <div className="flex flex-col justify-between">
                    <div>
                       ...
                       <div className="space-y-6 mb-16 italic uppercase font-bold text-[10px] tracking-widest opacity-60">
                          <a href="https://www.google.com/maps/search/?api=1&query=Av.+La+Dehesa+4589,+Lo+Barnechea" target="_blank" className="flex items-center gap-4 hover:text-[#E50000] transition-colors"><MapPin className="text-[#E50000]" size={20} /> Av. La Dehesa 4580, Local 8.</a>
                    ...
                    <div className="bg-black/40 backdrop-blur-xl border border-white/5 rounded-[4rem] p-8 md:p-10 shadow-2xl relative overflow-hidden group max-w-md mx-auto lg:mx-0">
                       ...
                       <form onSubmit={(e) => { e.preventDefault(); const formData = new FormData(e.currentTarget); const name = formData.get('name'); const email = formData.get('email'); const message = formData.get('message'); if (!name || !email || !message) return; window.location.href = `mailto:raionbjjacademy@gmail.com?subject=CONSULTA RAION BJJ - ${name}&body=Nombre: ${name}%0D%0ACorreo: ${email}%0D%0AMensaje:%0D%0A${message}`; }} className="space-y-8 relative z-10">
                          ...
                          <button type="submit" className="w-full bg-[#E50000] text-white py-4 rounded-full font-black uppercase tracking-[0.3em] text-[9px] hover:bg-white hover:text-black transition-all flex justify-center items-center gap-3 group/btn shadow-[0_0_20px_rgba(229,0,0,0.15)]">ENVIAR MENSAJE <ChevronRight size={16} className="group-hover/btn:translate-x-2 transition-transform" /></button>
                       </form>
                    </div>
                 </div>
                 {/* Columna Derecha: Mapa de Google (Grayscale Premium) */}
                 <a href="https://www.google.com/maps/search/?api=1&query=Jiu+Jitsu+Raion+Bjj" target="_blank" className="h-[500px] lg:h-auto min-h-[400px] relative rounded-[2rem] overflow-hidden border border-white/10 group shadow-2xl block">
                    <div className="absolute inset-0 bg-[#E50000]/10 pointer-events-none z-10 group-hover:opacity-0 transition-opacity duration-700" />
                    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3332.7224605972083!2d-70.5207373!3d-33.352199899999995!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x9662cb9f719d666d%3A0x21adc56746247152!2sJiu%20Jitsu%20Raion%20Bjj!5e0!3m2!1sen!2scl!4v1776652187682!5m2!1sen!2scl" className="w-full h-full grayscale invert contrast-[1.2] opacity-60 group-hover:grayscale-0 group-hover:invert-0 group-hover:opacity-100 transition-all duration-1000 border-none pointer-events-none" allowFullScreen={true} loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
                    <div className="absolute top-6 left-6 z-20"><span className="bg-black/80 backdrop-blur-md text-white px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border border-white/10 italic">Raion BJJ Academy</span></div>
                    <div className="absolute bottom-6 right-6 z-20 opacity-0 group-hover:opacity-10 transition-opacity"><span className="bg-[#E50000] text-white px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest italic flex items-center gap-2">ABRIR EN MAPAS <MoveUpRight size={12} /></span></div>
                 </a>
              </div>
      ...
      {/* --- ESTILOS GLOBALES: Fuentes y Animaciones Especiales --- */}
      <style jsx global>{`
